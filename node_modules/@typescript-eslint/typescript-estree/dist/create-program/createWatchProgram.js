 ...
 */
const tsconfigLsatModifiedTimestampCache = new Map();
...
function getProgramsForProjects(code, filePath, extra) {
    ...
    if (parsedFilesSeen.has(filePath) &&
        ...
        fileWatchCallbacks.forEach(cb => cb(filePath, typescript_1.default.FileWatcherEventKind.Changed));
    }
    /*
...
function createWatchProgram(tsconfigPath, extra) {
    ...
    const watchCompilerHost = typescript_1.default.createWatchCompilerHost(tsconfigPath, shared_1.DEFAULT_COMPILER_OPTIONS, typescript_1.default.sys, typescript_1.default.createSemanticDiagnosticsBuilderProgram, diagnosticReporter, 
    /*reportWatchStatus*/ () => { });
...
function hasTSConfigChanged(tsconfigPath) {
    ...
    const cachedLastModifiedAt = tsconfigLsatModifiedTimestampCache.get(tsconfigPath);
...
function maybeInvalidateProgram(existingWatch, filePath, tsconfigPath) {
    ...
    if (hasTSConfigChanged(tsconfigPath)) {
        ...
        fileWatchCallbackTrackingMap
            .get(tsconfigPath)
            .forEach(cb => cb(tsconfigPath, typescript_1.default.FileWatcherEventKind.Changed));
    ...
    let current = null;
    let next = path_1.default.dirname(filePath);
    let hasCallback = false;
    while (current !== next) {
        current = next;
        const folderWatchCallbacks = folderWatchCallbackTrackingMap.get(current);
        if (folderWatchCallbacks) {
            folderWatchCallbacks.forEach(cb => cb(current, typescript_1.default.FileWatcherEventKind.Changed));
            hasCallback = true;
            break;
        }
    ...
    fileWatchCallbacks.forEach(cb => cb(deletedFile, typescript_1.default.FileWatcherEventKind.Deleted));
